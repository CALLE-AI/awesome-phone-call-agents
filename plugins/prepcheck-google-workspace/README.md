...
It does not book, cancel, or move appointments. A released slot is a flag and
an email; rebooking stays with a person.
...
end to end and dials nobody: each would-be call is written to the `CallLog`
sheet as a `DRY_RUN` row containing the exact payload, including the full task
prose and the result schema. Read that before enabling calls — it is what
CALL-E will work from.

The whole state machine can also be exercised without spending a call by
posting the files in [`examples/`](examples/) to the deployed webhook:
...
Post `01` then `02` in order to see a regression detected and staff emailed,
with `prep_status` identical on both calls.
...
- **A patient asks not to be called**: the task prompt instructs the agent to
  agree and end. Clear `next_call_at` and set `STAFF_REVIEW` so no later sweep
  picks them up.
...
And enforced structurally: the result schema has no field capable of carrying
clinical content. `flag_for_staff` is a boolean. There is nowhere for a symptom
to be stored even if one were volunteered — see
...
| `NOT_PREPARED` | Cannot be ready in time | Slot released, staff emailed |
