...
| [`odoo-calle`](odoo-calle/) | Odoo | Odoo 19 module connecting server actions and automation rules to CALL-E voice workflows with prompt previews, E.164 normalization, structured results, evidence logs, and post-call action dispatches. |
...
| [`airtable-calle-certa`](airtable-calle-certa/) | Airtable | Consent-bound Verification of Employment run from an Airtable view: the recipient result schema is derived from the base's own field definitions, consent is a per-row hash a filled-down cell cannot forge, the applicant-supplied employer number is never dialed, and nothing is reported as verified without extracted evidence that a person was reached. |
| [`firstbell-absence-calls`](firstbell-absence-calls/) | n8n | Importable school-absence call wave that dials each family in that family's own language, keeps a call that connected without producing a usable answer separate from one that did, and ships with a dry run that places no calls. |
