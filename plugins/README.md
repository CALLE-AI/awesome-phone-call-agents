...
| [`airtable-calle-nominee`](airtable-calle-nominee/) | Airtable | Consent-bound Verification of Employment run from an Airtable view: the recipient result schema is derived from the base's own field definitions, consent is a per-row hash a filled-down cell cannot forge, the applicant-supplied employer number is never dialed, and nothing is reported as verified without extracted evidence that a person was reached. |
