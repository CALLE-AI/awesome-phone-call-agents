...
- Current phase: Complete the human-disposition loop before deployment
...
- `pnpm validate` passes locally, including type-check, lint, 84 unit tests,
  Drizzle schema validation, 49 PostgreSQL integration tests, production build,
  and 13 Playwright tests;
- public fake-only deployment instructions and a build-time safety gate exist;
- no judge-accessible deployment, protected staging deployment, authorized live
  CALL-E result, upstream pull request, or final three-minute video is claimed.

P0A is complete. The remaining work starts with P0B deployment and protected
staging, followed by authorized live evidence and submission packaging. The only
later product convenience is a preset fictional work order for the
demonstration; it must use the existing case workflow and must not create a new
stage.
...
| Item | Status | Done when |
| --- | --- | --- |
| Deploy a judge-accessible fake-only public version | Pending | A stable HTTPS URL passes the full judge smoke test in [Public Fake-Only Deployment](public-demo-deployment.md), contains no CALL-E credentials, and completes through recorded human disposition |
| Establish an isolated protected staging environment | Pending | Staging has a separate database and secrets, access is allow-listed, live calls default to paused, and the kill switch is verified before credentials are enabled |
| Complete one authorized real CALL-E test | Blocked by staging and participant authorization | One exact recipient, brief, timezone, and test window are approved; one call is accepted; one terminal result is retrieved; the operator records the disposition; and no duplicate call is created |
...
| Stabilize the three-minute golden path | Pending deployment and rehearsal | The path uses a preset case, ends with a recorded disposition, completes within three minutes in three consecutive rehearsals, and labels every waiting-time edit accurately |

### P0 dependency order

Steps 1–5 are complete. P0B begins at step 6.
