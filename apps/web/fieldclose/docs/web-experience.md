...
After the server accepts the call, the create action locks rather than offering a retry. The workbench displays the provider call identifier, `Status polling active`, and `Checking CALL-E status`, then asks the FieldClose backend to refresh the existing call every five seconds before loading redacted case state. At 600 seconds, an unresolved final lookup moves the case to manual reconciliation; the operator can still refresh that same call later without redialing.
