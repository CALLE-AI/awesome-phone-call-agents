...
describe("CALL-E provider adapter", () => {
  ...
  it("directs emergencies to local emergency services and keeps advice boundaries explicit", async () => {
    let capturedRequest: Request | null = null;
    const provider = createProvider(async (request) => {
      capturedRequest = request;
      return Response.json(callResponse({ status: "queued" }));
    ...
    await provider.createCall(createRequest());
    const body = (await (capturedRequest as unknown as Request).clone().json()) as {
      task: string;
    };

    expect(body.task).toContain("local emergency services");
    expect(body.task).toContain("911");
    expect(body.task).toContain(
      "The contractor callback is never the emergency path",
    );
    expect(body.task).toContain("Never provide medical, legal, or financial advice");
  });
