...
import { lockRecipientSuppression } from "@/application/recipient-suppression";
    ...
    if (normalized.doNotCallRequested) {
      const [closeoutCase] = await transaction
        .select({
          workspaceId: closeoutCases.workspaceId,
          contactId: closeoutCases.contactId,
          phoneLookupHash: contacts.phoneLookupHash,
        })
        .from(closeoutCases)
        .innerJoin(contacts, eq(contacts.id, closeoutCases.contactId))
        .where(eq(closeoutCases.id, caseId))
        .limit(1);
      ...
      await lockRecipientSuppression(
        transaction,
        closeoutCase.workspaceId,
        closeoutCase.phoneLookupHash,
      );
      ...
      await transaction
        ...
        .where(
          and(
            eq(contacts.workspaceId, closeoutCase.workspaceId),
            eq(contacts.phoneLookupHash, closeoutCase.phoneLookupHash),
          ),
        );
