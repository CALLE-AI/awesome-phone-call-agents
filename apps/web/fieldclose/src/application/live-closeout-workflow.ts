...
async function prepareLiveStatusRefresh(
  ...
  return db.transaction(async (transaction) => {
    await requireProtectedRefreshTransactionAccess(
      transaction,
      userId,
      workspaceId,
    );
...
async function prepareLiveProviderRequest(
  ...
  return db.transaction(async (transaction) => {
    ...
    if (attempt.providerCallId || attempt.requestedAt) {
      return {
        kind: "existing" as const,
    ...
    if (!claimedAttempt) {
      throw new Error("The approved live attempt could not be claimed");
    }
...
async function requireProtectedRefreshTransactionAccess(
  transaction: LiveWorkflowTransaction,
  userId: string,
  workspaceId: string,
) {
  ...
  if (access.role === "auditor") {
    throw new WorkflowPolicyError(
      "operator_role_forbidden",
      "An auditor cannot refresh provider status",
    );
  }
}
