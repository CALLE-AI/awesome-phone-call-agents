...
export function normalizeProviderSnapshot(
  ...
  if (
    ...
    snapshot.taskStatus === "canceled"
  ) {
    return {
      ...normalizeTerminalProviderFailure(snapshot),
  ...
  return {
    ...
    doNotCallRequested:
      parsed.data.contactVerification === "refused" ||
      parsed.data.escalationReasons.includes("do_not_call_requested"),
  };
}

function normalizeTerminalProviderFailure(
  snapshot: ProviderCallSnapshot,
): NormalizedCallResult {
  return {
    providerCallId: snapshot.providerCallId,
    ...
    administrativeResults: {},
    outOfScopeTopics: [],
    escalationReasons: [`provider_${snapshot.taskStatus}`],
    summary: "The provider task failed and cannot produce a closeout result.",
    evidenceRefs: [],
    route: "failed",
    normalizerVersion: "fieldclose-v1",
    validationFailed: false,
