...
import { readDriveRequest, assertLiveWindow } from "./request.js";
import { reservePrivateReport } from "./report.js";
import { runDrive } from "./workflow.js";
...
async function main(): Promise<void> {
  ...
  const request = await readDriveRequest(args.request);
  if (args.command === "preview") {
    console.log(formatPreview(request));
    return;
  }
  if (!args.live || !args.confirmAuthorized || !args.receipt || !args.output) usage();
  if (args.receipt !== previewReceipt(request)) throw new Error("The receipt does not match the current request. Preview again; no call was placed.");
  ...
  try {
    ...
    const report = await runDrive(request, port, { checkpoint: (value) => reservation.checkpoint(value) });
    await reservation.finalize(report);
    console.log(`Drive finished. Redacted, unverified report written to ${args.output}.`);
  } catch (error) {
