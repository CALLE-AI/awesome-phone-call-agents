...
export function recipientResultSchema(request: SearchRequest): Record<string, unknown> {
  ...
  return {
    ...
    properties: {
      ...
      reference_code: { type: "string", description: "Optional non-sensitive desk reference code, otherwise an empty string. It must contain a letter and at most six digits; never return a phone, card, account, identity, or claimant value." },
      retrieval_mode: { type: "string", description: "Public, human-led follow-up route stated by the desk.", enum: ["official-channel", "in-person-desk", "posted-form", "unknown"] },
