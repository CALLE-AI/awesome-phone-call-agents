...
-- The observed provider needed 59.060 seconds to deliver its first signed callback.
-- Preserve the one-call, zero-retry safety envelope while allowing bounded provider
-- scheduling variance before the synthetic Twilio endpoint is reached.
ALTER TABLE "live_simulator_authorizations"
  ...
  CHECK (
    ...
    OR (
      ...
      AND "terminal_deadline_seconds" BETWEEN 1 AND 120
    )
  );
