...
Scheduler diagnostics use newline-delimited JSON at `logs/call-scheduler.ndjson`. The log records schedule creation, cancellation, expiration, dispatch claims, provider request timing, acceptance and bounded failure codes. It masks destinations and excludes the call purpose, API key, raw provider response, provider call ID and idempotency key. The entire `logs/` directory is ignored by Git. To follow it during local testing in PowerShell, run `Get-Content logs/call-scheduler.ndjson -Wait` from this app directory.
