...
import { calleFollowupsEnabled, createCalleFollowups } from "@/lib/calle/followup-server";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const service = enabled ? createCalleFollowups() : undefined;
    ...
    return reply({ enabled, calls: service ? await service.list() : [], available: ids.map((id) => ({ reference: reference(id), label: id.slice(0, 14) + "…" })) });
  } catch {
