# Consent-first mediation

**Claim:** no substantive call can be placed until *both* parties have said yes
on a recorded consent call — not as policy, but because the state machine has
no path to a shuttle call before both consents are recorded. Every later call
re-discloses recording; declining, cancelling, and expiry are terminal states.

**Reference implementation:** `src/state.ts` (consent gates),
`src/runner.ts` (`pendingWork`), `src/renderer.ts` (the `SCRIPT` table).
Tests: `test/state.test.ts` → *"consent"*, `test/renderer.test.ts`.
...
earlier — the gate is structural, not conventional.
...
- Terminal means terminal: `pendingWork` returns
  `{ kind: "none" }` for every terminal state, so no call is ever rendered
  for a declined, cancelled, expired, settled, or impasse case.

### Dialing discipline around the gate

Two adjacent safeguards in this repo are part of the same story:

- The CLI cannot dial a real number by accident: the CALL-E client is a mock
  unless `--live` is passed *and* `CALLE_API_KEY` is set (`src/cli.ts`).
...
- **No verification of who answered.** Consent is given by whoever answers the
  party's phone. Voice is not identity — see the
...
- **Quiet hours and retry pacing are policy, not yet enforcement.**
  ...
  consider wiring the window check directly in front of your dial call.
