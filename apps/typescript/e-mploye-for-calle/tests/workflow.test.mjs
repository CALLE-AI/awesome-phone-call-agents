...
const make = () => {
  ...
  return { workflow: createWorkflow({ store: new JsonStateStore(path.join(directory, "state.json"), () => ({ version: 1, employees: [{ id: "emp-ana", name: "Ana", role: "Support", phone: "+14155550101", locale: "en-US", region: "MX" }], shifts: [{ id: "shift-1", employeeId: "emp-ana", date: "2026-09-07", startTime: "09:00", endTime: "17:00", role: "Support", status: "scheduled" }], jobs: [], approvals: [], events: [] })), provider, clock }) , provider };
};
...
const makeLive = () => {
  ...
  const config = {
    ...
    calleLiveEnabled: true, calleTestPhone: "+14155550104", calleTestRegion: "US", calleTestLocale: "en-US",
...
describe("E-mploye workflow engine", () => {
  ...
  it("starts live mode empty and only loads the server-authorized test phone", async () => {
    ...
    const loaded = context.workflow.configureLiveWorkspace({ workflowType: "appointment_management", name: "Ana", phone: "+14155550104", business: "Luna Studio", recordLabel: "Service appointment", date: "2026-09-07", startTime: "09:00", endTime: "10:00", region: "US", locale: "en-US" });
    expect(loaded).toMatchObject({ executionMode: "live", employees: [{ id: "live-contact", phone: "+141•••••104" }] });
    const preview = context.workflow.preview({ employeeId: "live-contact", shiftId: "live-record" });
    expect(preview.employee.phone).toBe("+141•••••104");
    ...
    expect(context.requests[0].body.recipients[0].phones).toEqual(["+14155550104"]);
    expect(context.workflow.state().employees[0].phone).toBe("+14155550104");
  ...
  it("rejects live workspace data that does not match the authorized destination", () => {
    const context = makeLive();
    expect(() => context.workflow.configureLiveWorkspace({ workflowType: "lead_follow_up", name: "Prospect", phone: "+14155550105", business: "Norte Services", recordLabel: "Discovery follow-up", date: "2026-09-08", startTime: "10:00", endTime: "11:00", region: "US", locale: "en-US" })).toThrow("authorized test phone");
    expect(context.workflow.state().employees).toHaveLength(0);
  });
  ...
  it("reads CALL-E attempt transcripts and blocks malformed alternate times", async () => {
    ...
    context.workflow.configureLiveWorkspace({ workflowType: "appointment_management", name: "Ana", phone: "+14155550104", business: "Luna Studio", recordLabel: "Service appointment", date: "2026-09-07", startTime: "09:00", endTime: "10:00", region: "US", locale: "en-US" });
    const created = context.workflow.createJob({ employeeId: "live-contact", shiftId: "live-record" });
    const jobId = created.jobs[0].id;
