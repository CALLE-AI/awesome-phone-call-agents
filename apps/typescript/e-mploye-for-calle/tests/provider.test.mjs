...
describe("CALL-E API provider", () => {
  ...
  it("constructs an authenticated idempotent create request", async () => {
    ...
    expect(request.url).toBe("https://api.example.test/v1/calls");
  ...
  it("reads a 200 SDK response and maps structured result and transcript fields", async () => {
    const fetchImpl = vi.fn(async (request) => {
      expect(request.url).toBe("https://api.example.test/v1/calls/call_123");
      return new Response(JSON.stringify({
        id: "call_123", object: "call_task", status: "completed", task: "Confirm the appointment.",
        recipients: [{ id: "recipient_1", phones: ["+14155550101"], locale: "en-US", region: "US", status: "completed", structured_result: { outcome: "confirmed" }, summary: "Confirmed.", attempts: [{ id: "attempt_1", phone: "+14155550101", status: "completed", started_at: null, completed_at: null, summary: "Confirmed.", transcript_turns: [{ speaker: "user", text: "Yes." }], provider_call_id: null, failure_code: null, failure_message: null }] }],
        structured_result: { outcome: "confirmed" }, summary: "Confirmed.", task_completed: true, completion_confidence: { score: 0.96, label: "high" }, evidence: ["Yes."], metadata: {}, failure_code: null, failure_message: null,
