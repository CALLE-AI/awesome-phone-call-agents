  ...
  it("reads a 200 SDK response and maps structured result and transcript fields", async () => {
    const fetchImpl = vi.fn(async (request) => {
      ...
      return new Response(JSON.stringify({
        ...
        recipients: [{ id: "recipient_1", phones: ["+14155550101"], locale: "en-US", region: "US", status: "completed", structured_result: { outcome: "confirmed", phone: "+14155550101" }, summary: "Confirmed.", attempts: [{ id: "attempt_1", phone: "+14155550101", status: "completed", started_at: null, completed_at: null, summary: "Confirmed.", transcript_turns: [{ speaker: "user", text: "Yes, call +14155550101." }], provider_call_id: null, failure_code: null, failure_message: null }] }],
        structured_result: { outcome: "confirmed", phone: "+14155550101" }, summary: "Confirmed.", task_completed: true, completion_confidence: { score: 0.96, label: "high" }, evidence: ["Yes, call +14155550101."], metadata: { phone: "+14155550101" }, failure_code: null, failure_message: null,
    ...
    expect(result.recipients[0].attempts[0].transcript_turns[0].text).not.toContain("+14155550101");
    expect(result.recipients[0].phones[0]).not.toContain("+14155550101");
    expect(result.structured_result.phone).not.toContain("+14155550101");
    expect(result.evidence[0]).not.toContain("+14155550101");
  });
