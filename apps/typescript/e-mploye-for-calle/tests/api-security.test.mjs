...
const protectedWorkflow = () => {
  const response = () => ({
    version: 1,
    employees: [{ id: "contact", phone: "+14155550106" }],
    jobs: [{ result: { contact_message: "Call +14155550106" }, evidence: ["+14155550106"], transcript: [{ text: "+14155550106" }] }],
...
describe("live API security boundary", () => {
  ...
  it("rejects a wrong bearer token and accepts the configured token", async () => {
    ...
    expect(valid.body.employees[0].phone).not.toContain("+14155550106");
    expect(valid.body.jobs[0].result.contact_message).not.toContain("+14155550106");
    expect(valid.body.jobs[0].evidence[0]).not.toContain("+14155550106");
    expect(valid.body.jobs[0].transcript[0].text).not.toContain("+14155550106");
  });
