...
function launchInput(body: Record<string, unknown>) {
  return { phone: String(body.phone ?? ""), consent: body.consent === true, idempotencyKey: String(body.idempotencyKey ?? "") };
}
...
</style></head><body><main><p>TEAMLINE</p><h1>When a conversation works better than a text.</h1><p>TeamLine is an AI Voice Communication Assistant for Coaches.</p><section class="banner"><strong id="mode"></strong><p>Every live call requires an explicit button click. Checking a result never places a call. TeamLine never redials automatically.</p></section><section><h2>Demo recipient</h2><form id="phone-form"><label>US phone number <input id="phone" type="tel" autocomplete="tel" placeholder="(202) 555-0142" required></label><label><input id="consent" type="checkbox"> I consent to each TeamLine demo call I explicitly request.</label><button>Use this number</button></form><p id="phone-state"></p></section><section><h2>1. Find something out</h2><p>Ask a fictional Athletic Director about Friday facility availability. TeamLine may gather facts and return requests; it may not reschedule, spend, or commit the team.</p><div class="actions"><button id="facility-start" disabled></button><button id="facility-check" class="secondary" hidden>Check existing call result</button><button id="facility-retry" hidden>Try the call again</button><button id="approve" hidden>Coach: approve practice change</button></div><div id="facility-result"></div></section><section><h2>2. Pass something along</h2><p>After coach approval, communicate only the approved change and collect attendance and transportation responses.</p><div class="actions"><button id="parent-start" disabled></button><button id="parent-check" class="secondary" hidden>Check existing call result</button><button id="parent-retry" hidden>Try the call again</button></div><div id="parent-result"></div></section><p><a href="https://youtu.be/2btXyqeA3Wg">Watch the TeamLine demo video</a> · <a href="https://teamline-judge-console.netlify.app/teamline/demo">Functional competition demo</a></p></main><script>
let state,phone='',ready=false,busy=false;const $=id=>document.getElementById(id);const post=async(path,body={})=>{const response=await fetch(path,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});const value=await response.json();if(!response.ok)throw new Error(value.error);return value};const callInput=()=>({phone,consent:true,idempotencyKey:crypto.randomUUID()});
async function load(){state=await(await fetch('/api/state')).json();render()}
function attemptText(value){if(!value)return'';if(value.status==='in_progress')return '<p class="result">Call accepted / in progress. Check result retrieves this same call and never starts another.</p>';if(value.status==='no_answer')return '<p class="result">The call was not answered. No conversation occurred.</p>';if(value.status==='failed')return '<p class="result">CALL-E reported a failed call. No automatic retry occurred.</p>';return '<pre class="result">'+escapeHtml(JSON.stringify(value.result,null,2))+'</pre>'}
function escapeHtml(value){return value.replace(/[&<>]/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[char]))}
function render(){const live=state.mode==='live';$('mode').textContent=live?'LIVE CALL-E MODE — call buttons place real outbound calls.':'SANDBOX MODE — local fixtures only; no telephone call is placed.';$('facility-start').textContent=live?'Place one Athletic Director call':'Start Athletic Director no-call fixture';$('parent-start').textContent=live?'Place one Parent call':'Start Parent no-call fixture';$('facility-start').disabled=busy||!ready||!!state.facility;$('parent-start').disabled=busy||!ready||state.decision!=='approved'||!!state.parent;$('facility-check').hidden=state.facility?.status!=='in_progress';$('facility-check').disabled=busy;$('parent-check').hidden=state.parent?.status!=='in_progress';$('parent-check').disabled=busy;$('facility-retry').hidden=state.facility?.status!=='no_answer';$('facility-retry').disabled=busy||!ready;$('parent-retry').hidden=state.parent?.status!=='no_answer';$('parent-retry').disabled=busy||!ready;$('approve').hidden=state.decision!=='awaiting_coach';$('approve').disabled=busy;$('facility-result').innerHTML=attemptText(state.facility);$('parent-result').innerHTML=attemptText(state.parent)}
...
async function action(path,isCall){if(busy)return;if(isCall&&state.mode==='live'&&!confirm('This places one real outbound CALL-E call. Continue?'))return;busy=true;render();try{state=await post(path,isCall?callInput():{});render()}catch(error){alert(error.message)}finally{busy=false;render()}}
$('facility-start').onclick=()=>action('/api/facility/start',true);$('facility-check').onclick=()=>action('/api/facility/check',false);$('facility-retry').onclick=()=>action('/api/facility/retry',true);$('approve').onclick=()=>action('/api/approve',false);$('parent-start').onclick=()=>action('/api/parent/start',true);$('parent-check').onclick=()=>action('/api/parent/check',false);$('parent-retry').onclick=()=>action('/api/parent/retry',true);load();
</script></body></html>`;
}
