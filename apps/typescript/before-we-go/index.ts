...
async function main(){
 if(command==='sample'){console.log(JSON.stringify(exportReview(sample('barrier'),'seek_confirmation',true),null,2));return}
 ...
 console.log(JSON.stringify({id,recipient:'ending '+phone.slice(-4),task:payload.task,notice:'Preview only. Expires in ten minutes. No call placed.'},null,2));return;
 }
 ...
 if(command==='status'){
 if(row.result){console.log(JSON.stringify({id,state:row.state,result:JSON.parse(row.result)},null,2));return}
 ...
 const saved=get(db,id);console.log(JSON.stringify({id,state:saved.state,result:saved.result?JSON.parse(saved.result):null},null,2));return;
 }
