...
import { idempotencyKey, parseCheckInRequest, parseCheckInResult, type CheckInRequest, type CheckInResult } from '../server/contract.js'
...
test('the CALL-E payload has one recipient and a closed result schema', () => {
  const input = buildCallInput(request) as { recipients: unknown[]; recipientResultSchema: { additionalProperties: boolean } }
  assert.equal(input.recipients.length, 1)
  assert.equal(input.recipientResultSchema.additionalProperties, false)
})

test('the result boundary rejects missing, unknown, and mistyped fields', () => {
  assert.deepEqual(parseCheckInResult(result), result)
  const { opt_out: _missing, ...missing } = result
  assert.throws(() => parseCheckInResult(missing), /Invalid CALL-E result field: opt_out/)
  assert.throws(() => parseCheckInResult({ ...result, transcript: 'private' }), /Unknown check-in result field/)
  assert.throws(() => parseCheckInResult({ ...result, wants_event_reminder: 'yes' }), /Invalid CALL-E result field/)
})
...
test('opt-out cancels the cadence before any other action', () => {
  const plan = decidePostCall(request.eventOptions.map((event) => event.id), { ...result, opt_out: true, wants_community_introduction: true })
...
test('lack of conversation consent stores nothing', () => {
  const plan = decidePostCall(request.eventOptions.map((event) => event.id), { ...result, permission_to_continue: 'unknown', confirmed_memory: 'private', selected_event_id: 'history-tea', wants_event_reminder: true })
...
test('community introduction requires an explicit participant request', () => {
  assert.equal(decidePostCall(request.eventOptions.map((event) => event.id), { ...result, wants_community_introduction: true }).primaryAction, 'community_introduction_review')
  assert.equal(decidePostCall(request.eventOptions.map((event) => event.id), result).primaryAction, 'routine_follow_up')
})

test('memory is saved only after confirmed read-back', () => {
  assert.equal(decidePostCall(request.eventOptions.map((event) => event.id), result).memoryToSave, 'The first tomato turned red.')
  assert.equal(decidePostCall(request.eventOptions.map((event) => event.id), { ...result, memory_readback_confirmed: 'unknown' }).memoryToSave, null)
})

test('only a verified offered event can enter reminder review', () => {
  const valid = decidePostCall(request.eventOptions.map((event) => event.id), { ...result, selected_event_id: 'history-tea', wants_event_reminder: true })
  assert.equal(valid.primaryAction, 'event_reminder_review')
  assert.equal(valid.eventToReview, 'history-tea')
  const invented = decidePostCall(request.eventOptions.map((event) => event.id), { ...result, selected_event_id: 'invented-event', wants_event_reminder: true })
