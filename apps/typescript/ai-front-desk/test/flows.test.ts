...
const LIVE_PHONE = "+15550009999"; // matches test/test.env LIVE_CALL_OVERRIDE_PHONE; live calls always route here
...
test("startBackfillForSlot: an untrusted result halts the waterfall (NEEDS_REVIEW) instead of advancing to the next candidate", async () => {
  ...
  const second = await makeContact(business.id, "Kim", "+15550000099");
