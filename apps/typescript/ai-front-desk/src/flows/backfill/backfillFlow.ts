...
export async function startBackfillForSlot(slotId: string): Promise<BackfillOutcome> {
  ...
  for (let index = 0; index < candidates.length; index += 1) {
    ...
    const acceptedResult = String(call.structuredResult?.["accepted"] ?? "ambiguous");

    if (acceptedResult === "ambiguous") {
      await prisma.waitlistEntry.update({ where: { id: entry.id }, data: { status: "NEEDS_REVIEW" } });
      attempts.push({
        waitlistEntryId: entry.id,
        contactName: entry.contact.name,
        callLogId: call.callLogId,
        accepted: false,
        reason: "Ambiguous outcome or voicemail.",
      });
      console.log(`[backfill] slot=${slotId} halted: Ambiguous outcome or voicemail.`);
      return { slotId, attempts, filled: false, bookedAppointmentId: null, haltedReason: "Ambiguous outcome or voicemail." };
    }

    const accepted = acceptedResult === "yes";
