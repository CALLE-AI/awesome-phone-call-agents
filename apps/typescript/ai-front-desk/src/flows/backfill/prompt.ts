...
export function buildBackfillTask(ctx: BackfillPromptContext): { task: string; resultSchema: JsonSchema } {
  ...
  const task =
    ...
    `If they decline, thank them and let them know they will stay on the waitlist. ` +
    `Be brief and polite. Do not give medical, legal, or financial advice. ` +
    `If you reach voicemail, do NOT book anything; leave a short message and report accepted as ambiguous. If their answer is unclear, report accepted as ambiguous.`;
  ...
  const resultSchema: JsonSchema = {
    ...
    properties: {
      accepted: {
        ...
        description: "Whether the contact accepted the offered slot. Voicemail or uncertainty counts as ambiguous.",
      },
      reason: { type: "string", description: "Brief reason if they declined or were ambiguous. Empty otherwise." },
    },
  };
