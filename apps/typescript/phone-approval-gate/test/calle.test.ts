...
import { GateApiError, assertTrustedBaseUrl, createSdkPort, parseAllowedHosts } from "../src/calle.js";
...
test("an https host nobody named is refused, because https is not trust", () => {
  ...
  // Named on purpose, so it goes through.
  assert.equal(
...
test("an allowed host is an exact hostname and a pattern is refused", () => {
  ...
  for (const pattern of ["*.example.com", ".example.com", "example.com/v1", "example.com:8443"]) {
    assert.throws(
      ...
      (error: unknown) => {
        assert.ok(error instanceof ConfigError, `expected ConfigError for ${pattern}`);
