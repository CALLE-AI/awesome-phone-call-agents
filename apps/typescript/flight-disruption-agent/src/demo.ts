...
const delay = Number(process.argv[2] ?? 240);
const disruption = desk.reportDelay("NA721-2026-09-20", delay, "a late inbound aircraft");
const first = desk.snapshot().disruptions[0];
if (!first) throw new Error("No disruption recorded");

console.log(`\nNA 721 CGK-SIN delayed ${delay} min -> ${first.bookings[0]?.quote.changeCase} change\n`);
for (const b of first.bookings) {
  ...
  console.log(`        move from ${idr(cheapestMove)} | refund ${idr(b.quote.refund.amount)} of ${idr(b.farePaid)}`);
}
