...
class OrderShieldEngine:
    ...
    async def verify_order(
        ...
        if scenario in ["cancel", "unreachable", "pending"]:
        ...
        if scenario == "pending" or (result.action == VerificationAction.NEEDS_REVIEW and not result.verified):
            new_status = OrderStatus.CALL_PENDING
        elif result.verified and result.action == VerificationAction.CONFIRM:
