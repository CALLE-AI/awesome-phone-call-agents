...
def mask_phone(phone: Optional[str]) -> str:
    """Mask phone numbers for privacy protection and policy compliance."""
    if not phone:
        return "••••••••"
    p = str(phone).strip()
    if len(p) <= 4:
...
def validate_ascii_e164(phone: str, allow_synthetic: bool = False) -> str:
    """Enforce standard ASCII E.164 phone formatting and reject synthetic test numbers in live runs."""
    cleaned = phone.strip()
    e164_pattern = re.compile(r"^\+[1-9]\d{6,14}$")
    if not e164_pattern.match(cleaned):
        raise ValueError(
            f"Phone destination '{cleaned}' must adhere to valid ASCII E.164 format (e.g., +919876543210 or +14155552671)."
        )
    if not allow_synthetic and ("55501" in cleaned or "55555501" in cleaned):
        raise ValueError(
            f"Synthetic default destination '{cleaned}' is not authorized for live PSTN routing. Please configure a verified phone number."
        )
    return cleaned
