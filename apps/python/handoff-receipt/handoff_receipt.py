...
DISPLAY_PHONE=re.compile(r"(?<![A-Za-z0-9])(?:\+?[0-9][0-9(). /-]{5,}[0-9])(?![A-Za-z0-9])", re.ASCII)
TERMINAL={"completed","failed","canceled"}
SENSITIVE_REDIRECT_HEADERS={"authorization","idempotency-key"}
...
class RejectCredentialRedirect(urlrequest.HTTPRedirectHandler):
    """Fail closed instead of forwarding authenticated requests across redirects."""
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        present={k.casefold() for k,_ in req.header_items()}
        if present & SENSITIVE_REDIRECT_HEADERS:
            raise urlerror.HTTPError(req.full_url, code, "credential-bearing redirect rejected", headers, fp)
        return super().redirect_request(req, fp, code, msg, headers, newurl)

OPENER=urlrequest.build_opener(RejectCredentialRedirect())
...
def redact_display(v): return DISPLAY_PHONE.sub("[REDACTED_PHONE]",str(v))
def safe_ref(v): return bool(re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._/#-]{2,79}",str(v or "").strip()))
def advisory(verdict,reason): return {"advisory":True,"verdict":verdict,"reason":reason}
...
def validate(r):
    ...
    for role in ("sender","receiver"):
        p=r[role]
        if not isinstance(p.get("phone"),str) or not E164.fullmatch(p["phone"]): raise ValueError(f"invalid {role} phone")
...
def sender_ready_for_receiver(s):
    return bool(s and s.get("reached")=="yes" and s.get("handoff_claimed")=="yes" and safe_ref(s.get("case_reference")))

def reconcile(s,r):
    if not s: return advisory("UNKNOWN","sender_evidence_unusable")
    if s["reached"]!="yes": return advisory("UNKNOWN","sender_not_reached")
    if s["handoff_claimed"]=="no": return advisory("SENDER_DENIES_HANDOFF","sender_explicitly_denies_transfer")
    if s["handoff_claimed"]!="yes" or not safe_ref(s["case_reference"]): return advisory("UNKNOWN","sender_handoff_unproven")
    if not r: return advisory("UNKNOWN","receiver_evidence_unusable")
    if r["reached"]!="yes": return advisory("UNKNOWN","receiver_not_reached")
    if s["case_reference"].casefold()!=r["case_reference"].casefold(): return advisory("REFERENCE_MISMATCH","references_differ")
    if r["case_recognized"]=="no": return advisory("HANDOFF_NOT_ACKNOWLEDGED","receiver_does_not_recognize_reference")
    if r["case_recognized"]!="yes": return advisory("UNKNOWN","receiver_recognition_unknown")
    if s["next_owner_claim"]=="receiver" and r["ownership_status"]=="owns_next_action": return advisory("ACKNOWLEDGED_MATCH","same_reference_and_receiver_ownership_align")
    if s["next_owner_claim"]=="receiver" and r["ownership_status"]=="not_owner": return advisory("OWNERSHIP_CONTRADICTION","ownership_claims_conflict")
    return advisory("HANDOFF_ACKNOWLEDGED_OWNERSHIP_UNRESOLVED","reference_acknowledged_owner_unresolved")
...
def call(r,role,task,schema,state_path):
    ...
    if op:
        ...
        try:
            with OPENER.open(req,timeout=30) as resp: created=json.loads(resp.read())
        except Exception:
    ...
    while time.time()<end:
        ...
        with OPENER.open(req,timeout=30) as resp: payload=json.loads(resp.read())
...
def view(r):
    return {"handoff_id":r["handoff_id"],"subject":redact_display(r["subject"]),"sender":{"name":redact_display(r["sender"]["name"]),"phone":mask(r["sender"]["phone"])},"receiver":{"name":redact_display(r["receiver"]["name"]),"phone":mask(r["receiver"]["phone"])}}
...
def main(argv=None):
    ...
    if not a.live:
        print(json.dumps({**view(r),"mode":"PREVIEW_NO_CALL","sender_task":redact_display(sender_task(r)),"side_effects":False},indent=2)); return
    ...
    if not sender_ready_for_receiver(s):
...
if __name__=="__main__":
    try:
        ...
        raise SystemExit("ERROR: "+redact_display(str(exc)))
