...
- **Call-to-request binding**: Before any call enters analysis, `_bind_call_to_request()` verifies the returned recipient phone matches what was dialed, that the response contains recipients and attempts, and that a non-empty transcript exists. Fails closed: missing phone, mismatched phone, or empty transcript all reject the call as `insufficient` — no hiring recommendation is produced from unbound data.
...
204 credential-free tests across three files — zero API keys needed:
