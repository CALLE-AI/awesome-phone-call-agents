...
Every number here is drawn from a range a telecoms regulator has reserved for
fiction, so nothing in this file can route to a real subscriber:

* ``+1 NPA 555 0100``-``0199`` - NANP fictional block (ATIS/NANPA). The samples
  also use 555 as the area code, which is not assignable.
* ``+44 20 7946 0xxx`` - Ofcom drama range for London landlines.
* ``+44 1632 960xxx`` - Ofcom drama range; 01632 is not a real UK area code.

The malformed values below are deliberately unroutable and are never dialed.
"""
import pytest
...
GOOD = "+15555550142"          # +1 (555) 555-0142  - NANP fictional 555-01XX
OTHER = "+15555550188"         # +1 (555) 555-0188  - NANP fictional 555-01XX
UK_LONDON = "+442079460000"    # 020 7946 0000      - Ofcom drama range
UK_DRAMA = "+441632960123"     # 01632 960123       - Ofcom drama range


class TestE164Validation:
    @pytest.mark.parametrize("value", [GOOD, UK_LONDON, UK_DRAMA, f" {GOOD} "])
...
class TestMasking:
    ...
    def test_never_returns_the_full_number(self):
        for value in (GOOD, OTHER, UK_LONDON):
