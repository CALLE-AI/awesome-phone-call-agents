...
A live call requires two independent confirmations:

* the destination must appear in REFCHECK_ALLOWED_DESTINATIONS, which is the
  operator asserting they own or are authorized to call that number; and
* --i-have-consent, which asserts the referee agreed to be called and the
  candidate authorized the reference check.

Phone numbers are masked in all output, in preview and live alike. Sample
numbers are in the reserved fictional +1 555 01xx range.
"""
...
from refcheck.phone import DestinationError, allowlist, mask, mask_all, normalize_e164
...
def main() -> int:
    ...
    p.add_argument("--to", default="+15555550100", help="referee phone, ASCII E.164")
    ...
    try:
        destination = normalize_e164(args.to)
    ...
    try:
        ...
        print(f"Invalid {'REFCHECK_ALLOWED_DESTINATIONS'}: {exc}", file=sys.stderr)
        return 2
    ...
    from refcheck.client import CredentialTargetError, build_request, resolve_base_url
    ...
    if not args.consent:
        print(
            ...
            "and the candidate authorized the reference check.",
            file=sys.stderr,
        )
        return 2
    ...
    try:
        ...
        return 2
    except (CredentialTargetError, RuntimeError) as exc:
    ...
    print(
        mask_all(json.dumps(result, indent=2))
        if result
        else "null - no schema-valid result"
    )
    if result:
