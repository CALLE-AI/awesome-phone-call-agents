...
`judge_proof.py` is a one-command, standard-library-only verification. It
regenerates `judge_bundle.json` through the real `rescue.run_rescue` engine,
requires an exact match with the committed judge artifact, checks both the
golden and fail-closed paths, and reproduces the impact model. It cannot place a
call.
...
**Engine audit** to inspect the dispatch, evidence, stop, and handoff events
generated by `rescue.py`, then download them as JSON. The committed
[`judge_bundle.json`](judge_bundle.json) is regenerated and exact-match checked
by the test suite, so the interface cannot silently substitute a different
decision path. The artifact contains no raw phone number, transcript, provider
...
The regressions cover exact engine-to-demo evidence generation, phone masking, consent and expiry validation,
