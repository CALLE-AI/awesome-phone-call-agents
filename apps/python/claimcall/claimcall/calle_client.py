...
returns a canned transcript and structured result so the full workflow runs offline.
"""
...
class CalleClient:
    def __init__(self, api_key: str, base_url: str = DEFAULT_BASE_URL, timeout: int = 30, allow_local_fake: bool = False):
        if not api_key:
            raise CalleError("CALLE_API_KEY is not set. Use preview or fixture mode for a no-call run.")
...
class FakeCalleServer:
    ...
    The first GET returns in_progress, the second returns the terminal fixture, so
    polling code is exercised the same way as against the real API.
    """

    def __init__(self, fixtures_dir: str, default_scenario: str = "cancelled-flight", host: str = "127.0.0.1", port: int = 0):
