...
CALLE_API_ENDPOINT = os.environ.get("CALLE_API_ENDPOINT", "https://api.heycall-e.com/v1/calls")

def format_e164(phone: str) -> str:
    """Formats phone number strictly into E.164 standard (+15550199, +15550199)."""
    if not phone:
        return "+15550199"
...
class CallEVoiceService:
    def __init__(self):
        ...
        logger.info(f"[Call-E Service] Call-E Voice API endpoint set to '{CALLE_API_ENDPOINT}'. Using CalleClient SDK: {CalleClient is not None}")
    ...
    def call_on_call_engineer(self, result_schema: dict):
        ...
        incident_result_schema = {
            "type": "object",
            "required": ["approved", "resolution_instructions", "evidence_summary"],
            "properties": {
                "approved": {
                    "type": "string",
                    "enum": ["yes", "no", "unknown"],
                    "description": "Whether the engineer explicitly approved running the recommended SQL remediation steps. Use yes when approved, no when declined, and unknown if unconfirmed."
                },
                ...
                "evidence_summary": {
                    "type": "string",
                    "description": "One concise sentence summarizing the engineer's response."
                }
            },
        ...
        logger.info("\n" + "="*75)
        logger.info(f"📞 [CALL-E PYTHON SDK OUTBOUND CALL] Target: On-Call Engineer {eng_name} ({phone})")
        logger.info("="*75)

        if self.api_key:
            try:
                if CalleClient is not None:
                    base_url = CALLE_API_ENDPOINT.replace("/v1/calls", "").replace("/calls", "").rstrip("/")
                    if not base_url:
                    ...
                    call_id = created_call.get("id") if isinstance(created_call, dict) else getattr(created_call, "id", created_call)
                    logger.info(f"[Call-E SDK] Call created via CalleClient SDK: id={call_id}")
                    if call_id:
                        logger.info(f"[Call-E SDK] Waiting for call '{call_id}' to complete...")
                        completed_call = client.calls.wait_for_result(call_id, timeout_seconds=300, interval_seconds=20)
                        logger.info(f"[Call-E SDK] Call '{call_id}' finished: status={completed_call.get('status')}, task_completed={completed_call.get('task_completed')}")
                else:
                    ...
                    if response.status_code in [200, 201, 202]:
                        ...
                        logger.info(f"[Call-E API] Call triggered via REST (HTTP {response.status_code}). id={call_id}")
                        if call_id:
                            logger.info(f"[Call-E API] Polling call status for '{call_id}'...")
                            import time
                            base_url = CALLE_API_ENDPOINT.replace("/v1/calls", "").replace("/calls", "").rstrip("/")
                            ...
                            while time.time() < deadline:
                                ...
                                if poll_res.status_code == 200:
                                    ...
                                    if status in ["completed", "failed", "canceled"]:
                                        logger.info(f"[Call-E API] Call '{call_id}' reached terminal status: {status}")
                                        break
                                time.sleep(2)
                    else:
                        logger.warning(f"[Call-E API] Call API request returned HTTP {response.status_code}: {response.text}")
    ...
    def call_post_validation(self, engineer_info: dict, dag_id: str, task_id: str, success: bool, zoom_bridge_url: str = None):
        ...
        logger.info("\n" + "="*75)
        logger.info(f"📞 [CALL-E PYTHON SDK FOLLOW-UP CALL] Target: {eng_name} ({phone})")
        logger.info("="*75)
        ...
        if self.api_key:
            try:
                if CalleClient is not None:
                    ...
                    call_id = created_call.get("id") if isinstance(created_call, dict) else getattr(created_call, "id", created_call)
                    logger.info(f"[Call-E SDK] Follow-up call created via CalleClient SDK: id={call_id}")
                    if call_id:
                        logger.info(f"[Call-E SDK] Waiting for follow-up call '{call_id}' to complete...")
                        completed_call = client.calls.wait_for_result(call_id, timeout_seconds=120, interval_seconds=2)
                        logger.info(f"[Call-E SDK] Follow-up call '{call_id}' finished: status={completed_call.get('status')}")
                else:
                    response = requests.post(
                        CALLE_API_ENDPOINT,
                        json={
                    ...
                    if response.status_code in [200, 201, 202]:
                        logger.info("[Call-E API] Follow-up call placed successfully via REST (HTTP OK).")
                    else:
                        logger.info(f"[Call-E API] Follow-up call REST status: HTTP {response.status_code}")
            except Exception as e:
