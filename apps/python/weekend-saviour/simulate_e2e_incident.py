...
def run_flask_incident_server():
    """Runs the Flask Incident Remediation Server with Error and Resolution Queues."""
    app.run(host='127.0.0.1', port=7071, debug=False)

def main():
    print("="*85)
    print("🚀 STARTING CALL-E AUTONOMOUS INCIDENT RESPONSE & LANGGRAPH REMEDIATION SIMULATION")
    print("="*85)
    ...
    print("[Simulation Engine] Re-running Sunday Night Retail ETL DAG task after LangGraph remediation...")
    dag_success = run_retail_etl_task()

    print("\n" + "="*85)
    if dag_success:
        print("🎉 SUCCESS: Call-E Incident Response & LangGraph Auto-Remediation Workflow Complete!")
        print("   - Initial DAG SQL Failure Enqueued to Flask Error Queue")
        print("   - Jira API Title/Description Search & Confluence Playbook Lookup Executed")
        print("   - Confluence Team Calendar & Local Config Fallback Checked (Alex Morgan contacted)")
        print("   - Call-E Outbound Voice Call Triggered (https://docs.heycall-e.com/calls)")
        print("   - Resolution Instructions Enqueued to Resolution Queue")
        print("   - LangGraph StateGraph & Gemini Tools Executed Schema Fix on SQLite DB")
        print("   - Airflow DAG Re-run Succeeded & Call-E Follow-up Confirmation Placed!")
    else:
