...
and linking to it is not obviously the same as removing it. The linked page serves eight
recordings and fifty nine transcript turns to anybody with the address and no account. So
here is the reasoning, and it is offered as reasoning rather than as permission.

The requirement, as the maintainer wrote it, is about what a contributor commits. A git
history is permanent, forkable and mirrored, and a recording of a real person that lands in
one cannot be taken back out; that is what the rule protects against, and this repository
contains none of it. What the page carries is not a third party's words. Every call on it
went from the author to the author, on a line the author owns, on numbers that cannot be
assigned to anybody else, to a script the author wrote, about a pupil who does not exist.
The consent register is committed and names the same person on both ends. There is nobody on
those recordings whose interest the rule exists to protect.

That is a reading, and the maintainer's is the one that counts. **If the requirement is meant
to cover a linked page as well as the tree, say so on the pull request and the recordings and
the turn by turn transcripts come down the same day.** The page is generated by
`tools/judge_page.py` in this repository and builds without the `--audio` flag, so removing
them is one argument, not a rewrite. Nothing this entry argues depends on the audio: the
structured results, the counts, the escalations and every figure on the reviewer page are
derived from the receipts, and the receipts are not the recordings.
