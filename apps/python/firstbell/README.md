...
![The path of one absence row: a gate on consent and then on voice reachability, where a family with no consent is never dialled and a family the telephone cannot reach goes to a person instead; then a short instruction in the family's own language with the automated-caller disclosure first; then CALL-E places the call under a cap on families, one key per row, polled to the end; then exactly one of three endings, resolved owned by nobody, undetermined and failed owned by a person; and a safeguarding escalation as a second axis leaving resolved and undetermined but never failed, because a call that reached nobody has no answer to read a rule against.](docs/images/the-path-of-one-absence.svg)
...
| Every gate here was broken on purpose to prove it fires | [`evidence/MUTATIONS.md`](evidence/MUTATIONS.md), 227 rows, each with the change made and the number of tests that noticed |
...
| 4 | [`evidence/MUTATIONS.md`](evidence/MUTATIONS.md) | Two hundred and twenty-seven gates broken on purpose, with how many tests noticed each one | 1 min |
...
python -m pytest tests/ -q          # 599 tests collected
python -m pytest tests/ -q -rs      # and the reason for every one that skips
```

**599 is the number collected, not the number that will pass on your machine.** Some of
