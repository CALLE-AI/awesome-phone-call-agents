...
| Every gate here was broken on purpose to prove it fires | [`evidence/MUTATIONS.md`](evidence/MUTATIONS.md), 316 rows, each with the change made and the number of tests that noticed |
...
| 4 | [`evidence/MUTATIONS.md`](evidence/MUTATIONS.md) | Three hundred and sixteen gates broken on purpose, with how many tests noticed each one | 1 min |
...
python -m pytest tests/ -q          # 774 tests collected
python -m pytest tests/ -q -rs      # and the reason for every one that skips
```

**774 is the number collected, and two different pairs add up to it.** Some of these gates
...
A clean checkout of this commit into an empty directory reports **742 passed, 32
...
reports **772 passed, 2 skipped**. Both pairs are measured, both add up to 774, and the
difference between them is what a reader has on their disk.

The very first run in a fresh clone reports one more skip and one fewer pass, 741 and 33.
