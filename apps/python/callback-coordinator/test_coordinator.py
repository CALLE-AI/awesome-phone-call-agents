...
def _completed(**overrides):
    ...
    base = {
        ...
        "recipients": [{"phones": ["+12025550123"], "locale": "en-US"}],
        "structured_result": {
            "right_person": "yes",
...
def test_execute_scheduled_ticket_masks_evidence():
    value = intake()
    client = FakeClient(_completed_for_intake(value))
...
def test_execute_fail_closed_on_low_confidence():
    value = intake()
    client = FakeClient(
        _completed_for_intake(value, completion_confidence={"score": 0.2, "label": "low"})
    )
...
def test_execute_fail_closed_when_create_has_no_id():
    ...
    value = intake()
    client = FakeClient(_completed_for_intake(value))
    client.calls = NoIdCalls(_completed_for_intake(value))
    ticket = coordinator.execute_with_client(value, client, now=_ny(14, 0), timeout_seconds=60)
    assert ticket["disposition"] == "needs_human"
...
def test_execute_fail_closed_on_binding_mismatch():
    value = intake()
    # Wrong workflow_id
    bad = _completed(
        metadata={"workflow_id": "other-workflow", "workflow_type": "callback_triage", "source": "web_form"},
        recipients=[{"phones": [value.phone]}],
    )
    client = FakeClient(bad)
    ticket = coordinator.execute_with_client(value, client, now=_ny(14, 0), timeout_seconds=60)
    assert ticket["disposition"] == "needs_human"
    assert "binding" in ticket["reason"]
    assert "workflow_id" in ticket["reason"]
    # Wrong recipient phone
    bad2 = _completed(
        metadata={"workflow_id": value.workflow_id, "workflow_type": "callback_triage", "source": "web_form"},
        recipients=[{"phones": ["+19998887777"]}],
    )
    client2 = FakeClient(bad2)
    ticket2 = coordinator.execute_with_client(value, client2, now=_ny(14, 0), timeout_seconds=60)
    assert ticket2["disposition"] == "needs_human"
    assert "binding" in ticket2["reason"]
    assert "recipient" in ticket2["reason"]
    # Call ID mismatch (id field differs from created id)
    bad3 = _completed_for_intake(value)
    bad3["id"] = "call-different-999"
    client3 = FakeClient(bad3)
    ticket3 = coordinator.execute_with_client(value, client3, now=_ny(14, 0), timeout_seconds=60)
    assert ticket3["disposition"] == "needs_human"
    assert "binding" in ticket3["reason"]
    assert "call_id" in ticket3["reason"]
def test_classify_needs_human_on_binding_mismatch():
    v = intake()
