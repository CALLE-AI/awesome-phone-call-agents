"""Idempotency keys derived from the authorization, not from the attempt.
...
def derive_idempotency_key(
    *,
    ...
    """Return a stable key for one authorized business operation.
    ...
    if not parts[1] or not parts[2] or not parts[3]:
        raise IdempotencyError(
            "authorization_record_reference, case_reference and contract_version "
            "are all required"
        )
