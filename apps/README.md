...
| [`web/nota-intake`](web/nota-intake/) | JavaScript / Cloudflare Pages | Law-firm client intake by phone: a three-step form (EN/ES/ID) picks a topic — real estate, will, trust, lawsuit — and CALL-E calls the client back within a minute to run that topic's interview with its own `recipient_result_schema`; a staff console places intake and reminder calls from the desk, follows them live, and syncs outcomes. Deployed and tested with real calls; documents the languages each line actually accepts. |
