...
| [`python/backline`](python/backline/) | Python | Calls suppliers in parallel for a price quote and ranks what comes back by landed cost, but never negotiates below a stated price and never commits to an order on the call — collecting a quote is the only thing the goal text permits, ordering is a separate human-approved step in the full project. A confirmation token bound to the item and the sorted number list gates `--execute`, and the canned `--simulate` responses are shaped from a real, live-verified completed call rather than the tool schema alone. |
