...
| [`python/known-number`](python/known-number/) | Python | Callback verification for vendor payment-detail changes: dials only the phone number already on file (and only if it predates the request), sends a task that contains no bank name or digits so the vendor reads the new details back, and reconciles the CALL-E result into one of five fail-closed verdicts with an audit memo. Preview, demo and tests place no call. |
