...
| [`python/known-number`](python/known-number/) | Python | Callback verification for vendor payment-detail changes: dials only the phone number already on file (and only if it predates the request), asks three yes/no questions and speaks a callback reference without collecting anything, then holds the change until a written reply from the address on file quotes that reference. Six fail-closed verdicts, audit memo, verified with a live call. Preview, demo and tests place no call. |
