...
// Validates fixture JSON files against the schema definitions.
...
const schema = require("../skills/insurance-claims-orchestrator/references/schema.json");
  ...
  for (const [field, fieldSchema] of Object.entries(definition.properties ?? {})) {
    ...
    if (fieldSchema.enum && value !== undefined && value !== null) {
      if (!fieldSchema.enum.includes(value as string)) {
        console.error(
          `  FAIL [${label}]: field "${field}" value "${value}" not in enum ${JSON.stringify(fieldSchema.enum)}`
        );
        passed = false;
      }
    }
