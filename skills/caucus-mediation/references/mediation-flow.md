...
`caucus_step_case`. Source of truth: the Caucus app in this repository —
`apps/typescript/caucus/src/state.ts` (transitions) and
`apps/typescript/caucus/src/runner.ts` (which call each state owes next).
