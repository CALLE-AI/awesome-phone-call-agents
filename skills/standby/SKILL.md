...
6. **Read the answer conservatively** using `references/reading-the-answer.md`. Anything that is not a clear yes is not a yes. A false acceptance ends the cascade and leaves the shift unfilled with everyone else already stood down; a false decline costs one extra call.
7. **On acceptance: stop.** Do not dial anyone else. Record the arrival time, and if it is after the shift starts, say so in minutes rather than reporting a clean fill.

End conditions: someone accepted; the list is exhausted; the shift started; or the user stopped it.

## Safety

- **One call in flight.** This is enforced, not advised. A second concurrent call is an error, not a slow path.
- **Numbers come from the user.** The skill never sources, guesses or completes a phone number.
...
A runnable dashboard implementing this skill, with a fixture mode that replays recorded calls so the whole cascade can be watched with no CALL-E account, is linked from `references/reference-implementation.md`.
