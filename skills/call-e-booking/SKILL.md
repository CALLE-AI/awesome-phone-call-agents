...
This skill wraps the `calle-ai` Python SDK (the Developer API — uses an API key, no browser OAuth needed) and orchestrates the end-to-end flow: gather booking parameters → show plan → confirm → dial → poll → structured result → Google Calendar event → notify.

The SDK is installed in the skill's own venv at `.venv/bin/python3` (Python 3.11).
Always run the scripts with that interpreter:

```bash
PY="$HOME/.hermes/skills/productivity/call-e-booking/.venv/bin/python3"
BOOK="$HOME/.hermes/skills/productivity/call-e-booking/scripts/calle_booking.py"
SLOTS="$HOME/.hermes/skills/productivity/call-e-booking/scripts/calendar_slots.py"
...
```bash
PY="$HOME/.hermes/skills/productivity/call-e-booking/.venv/bin/python3"
SCRIPT="$HOME/.hermes/skills/productivity/call-e-booking/scripts/calle_booking.py"
SLOTS="$HOME/.hermes/skills/productivity/call-e-booking/scripts/calendar_slots.py"
...
$PY $SCRIPT plan --kind garage --schema-file ~/.hermes/valet_schema.json \
...
- **API key**: get from https://dashboard.heycall-e.com/account/api-keys. Store in `~/.hermes/.env`:
  ```
  CALLE_API_KEY="iams_live_..."   # key prefix varies (iams_live_, calle_live_, etc.)
  ```
- **Callback number**: store your phone number in `~/.hermes/.env`:
  ```
  BOOKING_PHONE="+27XXXXXXXXX"
  ```
- **Python SDK**: installed in the skill's own venv (`.venv/bin/python3`, Python 3.11+). If the venv is missing, recreate it (see `references/venv-setup.md`). All scripts run via `$PY $SCRIPT`.
...
python3 $HOME/.hermes/skills/phone-scout/scripts/phone_scout.py \
...
clean, delegate the *execution* step to a subagent with `delegate_task`. The
...
delegate_task(
  ...
  context=(
    ...
    "PY=~/.hermes/skills/productivity/call-e-booking/.venv/bin/python3\n"
    "BOOK=~/.hermes/skills/productivity/call-e-booking/scripts/calle_booking.py\n"
    "$PY $BOOK book --confirm --kind restaurant --phone \"+1...\" "
    "--name \"Thethela Faltein\" --party-size 2 --caller-phone \"+27...\" "
    "--email \"...\" --pretty\n\n"
    "The script auto-loads CALLE_API_KEY from ~/.hermes/.env.\n"
    "If booked=true, also create the Google Calendar event:\n"
    "$PY ~/.hermes/skills/productivity/google-workspace/scripts/google_api.py "
...
- **Gather + confirm in the parent FIRST.** Subagents cannot ask the user (no
  `clarify`), so collect every detail and get explicit approval before
...
- **`.env` auto-load in background processes** — `calle_booking.py` uses `_load_dotenv()` to read `~/.hermes/.env` at startup, so `CALLE_API_KEY` and `BOOKING_PHONE` are available even when the shell didn't source it first. Background processes (started via `terminal(background=true)`) don't inherit the parent shell's exports, making this essential. Never remove this call.
