...
5. Immediately before calling `run_call`, acquire the dispatch lock (`scripts/call-lock.js`, `acquireLock`) keyed on the recipient's phone number and the purpose `"accesscall-intake"`. If a call to this recipient is already locked — including on a retry after a crash or timeout, which will see the same lock a first attempt wrote — refuse to place another and ask the user to explicitly confirm an override before retrying. The lock does not expire on a timer; it is only released in step 7, once a confirmed terminal status exists.
6. Call `run_call` with the `confirm_token` exactly as received. Never call it more than once for the same `plan_id`.
7. Poll `get_call_run` every 1-3 seconds while the run is active, then slow down, until a terminal status. As soon as one of `COMPLETED`/`FAILED`/`NO_ANSWER`/`DECLINED`/`CANCELED`/`CANCELLED`/`VOICEMAIL`/`BUSY`/`EXPIRED` is confirmed, call `scripts/call-lock.js`'s `releaseLock` with that status to release the dispatch lock from step 5. Never release it speculatively or because polling has taken a while — an unresolved status keeps the lock held until it either resolves or the user explicitly overrides.
...
- Acquire the dispatch lock (`scripts/call-lock.js`) immediately before every `run_call` and honor it, including on retries after a crash or timeout. It only releases on a confirmed terminal status from `get_call_run`, never on a timer — never place a duplicate call to the same recipient while it's held without an explicit user override.
...
- [`scripts/call-lock.js`](scripts/call-lock.js) — durable, file-based dispatch lock (atomic acquire, released only on a confirmed terminal call status) preventing a duplicate call to the same recipient
