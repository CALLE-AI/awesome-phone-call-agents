...
| [`apps/python/ringback`](apps/python/ringback/) | Python | Fills appointment slots that would otherwise stay empty: five campaign kinds — reminder, confirmation, rescheduling, booking, and a freed slot offered down a waiting list until the first firm yes ends the run and nobody else is disturbed. Phone numbers masked everywhere, "do not call again" honoured, simulation by default behind three independent locks, and a French/English switch that also carries the agent's briefing, the spoken dates and the voice. |
