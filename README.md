...
- [warranty-claimcall](apps/python/warranty-claimcall/) - Compiles invoice and warranty evidence into one consented, safety-gated CALL-E service-desk call, then verifies the case reference against the transcript; dry-run by default.
...
| [`apps/python/warranty-claimcall`](apps/python/warranty-claimcall/) | Python | Warranty service-desk CALL-E app: evidence → hashed allowlist → plan-bound consent → idempotent dial → transcript-verified case reference; fixture dry-run by default. |
