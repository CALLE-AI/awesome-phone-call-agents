...
- [Frontline Drift](https://github.com/millerandmuller/frontline-drift) - Compliance-audit app that places one CALL-E call per operator-owned location with a shared policy question, judges each transcript against the operator's own policy line via CALL-E's per-call result schema, and reports a location that never answers as NONRESPONSIVE rather than as a violation.
