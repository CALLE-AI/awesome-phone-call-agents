...
- [Callstack](https://github.com/RolexAlexander/callstack) - External codebase-ingesting technical support agent: point it at any repo (local path or git URL), it reads the real README and source, diagnoses a customer's issue grounded in what's actually there, then hands CALL-E a task to place the support call. Verified against two unrelated real repos to confirm it's genuinely generic. No-call preview is the default; a real call requires an explicit `--dispatch` flag after human review of the diagnosis and call task.
