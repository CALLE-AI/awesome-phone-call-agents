...
| [`apps/typescript/recover`](apps/typescript/recover/) | TypeScript | Voice payment recovery agent that calls subscribers on failed charges to capture a live decision (retry, update card, or pause) with deterministic safety gates, exact call preview approval, authoritative webhook re-fetch, and masked data privacy. |
