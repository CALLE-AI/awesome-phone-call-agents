...
- [Dial](apps/typescript/dial/) - Turns a plain-English request into goal-driven CALL-E calls: finds local businesses, calls until it has enough comparable answers, compares only what was actually said, and books with explicit authorization; no-call simulator is the default, and a business mode runs opt-out-aware, calling-hours-bound customer calls with structured outcomes.
...
| [`apps/typescript/dial`](apps/typescript/dial/) | TypeScript | Goal-driven phone work over CALL-E: finds businesses, calls until it has enough comparable answers, shows why it stopped and why a cheaper option lost, and books only with authorization. Includes an opt-out-aware, calling-hours-bound business mode with spreadsheet import and per-recipient structured outcomes. No-call simulator by default. |
