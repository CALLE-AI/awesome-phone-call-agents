...
- [Tariflow Support Agent](https://github.com/RolexAlexander/tariflow-call-e-support) - External codebase-grounded technical support agent: diagnoses a customer's issue from facts read directly out of the product's real source (size thresholds, cache TTLs, rate limits, queue names), then hands CALL-E a task to place the support call. No-call preview is the default; a real call requires an explicit `--dispatch` flag after human review of the diagnosis and call task.
