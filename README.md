...
| [`apps/typescript/attestcall`](apps/typescript/attestcall/) | TypeScript | Turns a vendor compliance question into a dated attestation record: one disclosed, consent-gated CALL-E call captures framework status, certificate expiry, and auditor, classifies the outcome fail-closed to a human on any ambiguity, and seals it into a SHA-256 hash-chained audit log. No-call demo mode by default. |
