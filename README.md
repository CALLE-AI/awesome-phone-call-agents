...
| [`apps/python/firstbell`](apps/python/firstbell/) | Python | Calls the families whose absence notification went unanswered, each in the language that family speaks, and keeps a call that connected without producing a usable answer separate from one that did, so a null structured result reaches a person instead of a coverage percentage. Runs offline against a bundled CALL-E double by default, and a receipt says `live` only when the host it dialled was the real API. [Evidence page](https://firstbell-evidence.vercel.app) |
