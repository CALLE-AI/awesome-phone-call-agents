...
- [`medicaid-exemption-screener`](skills/medicaid-exemption-screener/) - Screen consented benefits enrollees by phone for exemptions from the 2027 Medicaid work requirement before their coverage check date: clear whoever the state's data already clears without a call, ask only the questions their record has not answered, and return caseworker-reviewed worklist items rather than determinations. See the [evidence and architecture guide](docs/still-covered/).
...
| [`apps/typescript/still-covered`](apps/typescript/still-covered/) | TypeScript | Medicaid work-requirement exemption screener: clears whoever the state's own data already clears without a call, phones the rest in their language to explain the 2027 rule and ask only the exemption questions their record has not answered, classifies fail-closed, and turns every call into a caseworker-reviewed worklist. See the [evidence and architecture guide](docs/still-covered/). |
