...
- [Flight Disruption Agent](apps/typescript/flight-disruption-agent/) - Flight disruption desk for OTAs and airlines: prices every option across the airline, distributor, and OTA chain before dialing, has CALL-E offer those exact amounts to each affected passenger, and applies only clear, consented choices after the call, with a dry-run default and fictional data.
