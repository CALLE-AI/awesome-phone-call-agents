...
- [`authority-signoff-call`](skills/authority-signoff-call/) - Real-world accountability call for autonomous government/emergency-response agents: when a system auto-authorizes an action under a real delegated-authority tier without waiting for a human, this reaches the accountable official directly by phone for a real, attributable confirm/override. Safe by default: no CALLE_API_KEY/phone/enable flag means no real call, ever. Proven in production inside [GovOS](https://github.com/shubhangi-mish/agents-for-humans/tree/main/govos), a live autonomous emergency-response system running Delhi's real government response chain.
