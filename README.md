...
- [`relayme`](skills/relayme/) - Text-first phone relay for deaf, hard-of-hearing, or non-speaking users: places one disclosed CALL-E call, returns the conversation as a readable thread plus a fail-closed result, with reserve-before-dial and no fabricated answers.
...
| [`apps/python/relayme`](apps/python/relayme/) | Python | Text-first phone relay for deaf/HoH/non-speaking users: renders a call as an accessible chat thread, reserves before dialling, classifies results fail-closed with transcript grounding, and ships a single-file web view with a no-call default. |
