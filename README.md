...
| [`apps/typescript/rebookrelay`](apps/typescript/rebookrelay/) | TypeScript | No-show recovery platform that detects calendar no-shows via Google Calendar, dispatches retention calls via CALL-E, and cascades open slots down a prioritized waitlist using an Inngest durable state machine. Dry-run demo on dashboard. |
