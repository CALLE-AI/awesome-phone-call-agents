...
| [`apps/web/call-review-console`](apps/web/call-review-console/) | TypeScript / Next.js | Reads how a call ended across the Calls API, Goal Runs and MCP, and keeps three answers apart: how the call ended, whether the job got done, and whether usable data came back. Every answer names the field it came from and whether it was stated or worked out, so a task reported complete on a call that never reached a person routes to a human instead of a success branch. Paste a payload, paste a key, or copy a webhook URL; the paste path needs no credentials and it cannot place a call. |
...
- [`What a surface can tell you`](apps/web/call-review-console/docs/what-a-surface-can-tell-you.md) - Measured from live calls: the top-level failure code repeats the status while the ending sits in a nested attempt code, failure_message reports a busy line as NO ANSWER, task_completed reads true for an answering machine, completion_confidence rises as the call gets less useful, and a terminal status can arrive before the result does.
