...
| [`apps/typescript/roll-call`](apps/typescript/roll-call/) | TypeScript | First-hour absence verification for schools: dials the guardians of each unexplained absentee in cascade order, keeps a "guardian did not know" verdict only when a transcript turn supports it, escalates that child to a named safeguarding contact, and never dials anybody twice in a day; preview and dry run by default, scripted fake included. |
