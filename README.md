...
| [`apps/typescript/voice-sre`](apps/typescript/voice-sre/) | TypeScript | Autonomous hands-free production incident hotfix agent that dials on-call engineers via CALL-E during P0 outages, delivers concise root-cause briefings, captures verbal remediation decisions via structured result schemas, and executes automated Git and Kubernetes rollbacks with a full audit trail and dry-run preview mode. |
