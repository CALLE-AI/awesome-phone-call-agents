...
- [`authority-signoff-call`](skills/authority-signoff-call/) - Post-hoc phone confirm/override for a decision an autonomous agent already auto-authorized under a real delegated-authority tier, reaching the accountable person even when they're away from a dashboard. `override` must not automatically unwind a real emergency or financial action on its own — see the skill's rules. Safe by default: no CALLE_API_KEY/phone/enable flag means no real call, ever. Demonstrated end to end, including a real placed call, inside [GovOS](https://github.com/shubhangi-mish/agents-for-humans/tree/main/govos), an experimental incident-response simulation built for a separate hackathon.
