...
- [`raktdaan`](skills/raktdaan/) - Blood-donor recall for a named component shortage that screens a consented register for interdonation interval, deferral windows and per-component group compatibility before dialling, calls the least substitutable donor last, counts a confirmation only when the donor states an arrival window, and reports a suppression histogram of everyone it deliberately did not call.
...
- [`Courtesy-yes grading`](skills/raktdaan/references/reading-the-answer.md) - Why an answered "yes" is not a commitment in every calling culture, grading a reply on whether it contains a clock time rather than agreement, hedge markers in English, Hindi and Tamil, and reporting unclear separately from declined so an ambiguous answer is never recorded as a refusal.
- [`Recall register safety`](skills/raktdaan/references/safety.md) - Purpose-limited recall consent, opt-out checked before dialling and written back mid-run, why emotional coercion produces agreement that does not arrive, per-donor call budgets against register fatigue, and designing sequential dispatch around a platform with no cancel-in-flight operation.
