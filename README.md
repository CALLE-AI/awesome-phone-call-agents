...
| [`apps/typescript/recover`](apps/typescript/recover/) | TypeScript | Order/exception follow-up app that calls a subscriber about a failed payment and captures a live decision (retry, update card, or pause), with a no-call preview of the exact task and recipient shown before any call is placed, explicit human confirmation required to proceed, and a cancel path that reverts the subscriber with no call ever placed. |
