...
- [Post-Discharge Check](https://cloudnewbie.github.io/PDC/) - Care-team web app where scheduled CALL-E check-in calls to discharged patients surface red-flag symptoms with confidence scores mid-call, escalate to an on-call nurse with transcript context, and return structured results; runs a no-call demo mode by default.
...
| [`apps/typescript/post-discharge-check`](apps/typescript/post-discharge-check/) | TypeScript / React | Consent-gated post-discharge check-in calls for care teams: cadence-scheduled CALL-E calls, mid-call red-flag surfacing with quotes and confidence, nurse-owned escalation, structured results, and a no-call demo default. |
