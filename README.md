...
| [`apps/typescript/recover`](apps/typescript/recover/) | TypeScript | Order/exception follow-up app that calls a subscriber about a failed payment and captures a live decision (retry, update card, or pause) via CALL-E's structured Calls API, with a no-call preview of the exact task and recipient before any call is placed, explicit human confirmation required, capped automatic follow-up scheduling on no-answer only (never on ambiguous results), and a cancel path at any point in the chain. |
