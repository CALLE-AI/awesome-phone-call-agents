...
- [`hazard-roll-call`](skills/hazard-roll-call/) - Run a hazard-triggered welfare roll call over the phone: risk-ordered CALL-E batch waves with a fail-closed triage schema, escalation calls to emergency contacts for red and unreachable people, human-approved dispatch, and a reproducible after-action report; drills by default.
...
| [`apps/typescript/canopy`](apps/typescript/canopy/) | TypeScript | Hazard-triggered welfare roll call: when a heat, flood, outage or smoke alert fires, one CALL-E call task per risk-ordered wave triages every opted-in vulnerable person with a fail-closed schema, unreachable and red people escalate to a human contact by phone, and an append-only ledger produces the after-action report. Dry-run against a local fake CALL-E server by default. |
