...
- [`client-persona-profiler`](skills/client-persona-profiler/) - Post-call persona detection skill that classifies a caller's behavioural archetype (DISC model), computes an RFMAP loyalty score across interactions, persists a privacy-preserving profile, and returns a personalised next-call strategy playbook grounded in peer-reviewed persona and loyalty research.
- [`call-fraud-shield`](skills/call-fraud-shield/) - Real-time and post-call fraud detection skill that scores a transcript for vishing, spam, social engineering, deepfake voice, and scam-script patterns using LLM-based conversational trajectory analysis, returns XAI-explained evidence spans, and recommends a structured action — grounded in Vishing-Tactics-Bench, VishGPT, and adversarial-transcript research.
