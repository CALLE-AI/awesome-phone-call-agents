...
- [`ghost-network-audit`](skills/ghost-network-audit/) - Audit a health-plan provider directory for ghost listings by calling each listed office once with four administrative questions, then score coverage, ghost rate, and how many listings a new patient could actually use; unreached and ambiguous calls are reported as unverified rather than counted as findings.
