...
| [`apps/typescript/claimline`](apps/typescript/claimline/) | TypeScript | Multi-party insurance claim investigation: calls the claimant, treating doctor, hospital billing, and repair shop for schema-validated facts, aggregates a case total, runs a cross-party fraud/consistency check, and routes a human approve/hold/reject plus payout — with autopilot, Slack/Teams reports, masked numbers, fail-closed dispositions, an MCP server, English/Hindi/Spanish, and a no-call fixture default. |
