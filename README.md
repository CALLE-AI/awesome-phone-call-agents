...
- [`warranty-recovery`](skills/warranty-recovery/) - Resolves a warranty or entitlement exception the portal refused with one authorized call, and promotes an RMA or authorization reference only after the representative confirmed a spoken read-back, so a misheard digit downgrades the resolution instead of shipping against somebody else's authorization.
...
| [`python/warrantyops`](apps/python/warrantyops/) | Python | Warranty exception resolver whose most dangerous field is guarded by its own state machine: an authorization reference stays UNCONFIRMED until a read-back is performed, answered affirmatively, and that answer is found in the counterparty's own transcript turns, and extraction confidence is not an accepted input. Transport failure never becomes a coverage decision, and the default path replays synthetic fixtures with zero calls. |
...
- [`High-consequence identifier confirmation`](skills/warranty-recovery/references/identifier-confirmation.md) - Why extraction confidence cannot establish an RMA, case or booking reference, and the read-back state machine that can: an affirmative answer grounded in the counterparty's own transcript turns, or no reference at all.
