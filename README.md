...
- [CALL-E Code Commander](apps/typescript/call-e-code-commander/) - On-call escalation commander that phones the engineering rota during a production incident, explains the incident conversationally, and returns the engineer's spoken decision as a strict structured result that gates the deploy; simulation by default and nothing reaches production without a named human on the line.
